	...
	*/
	...
	public function updateInventory($test_id = null, $items_id = null, $amount_used = null, $quantity = null, $user_id = null){
		if($test_id){
			...
			$query = $this->db->insert('items', $update_data);
					
			return ($query === true ? true : false);	
		}return false;
	}

	public function activateTransaction($queue_id = null, $patient_id = null, $test_id = null, $user_id = null){
		if($test_id){
			$update_data = array(
				'trans_id'			=> $queue_id,
	...
	public function activateResult($queue_id = null, $patient_id = null, $test_id = null, $user_id = null){
		if($test_id){
	...
	public function salesLog($test_id = null, $user_id = null)
	{
		if($test_id){
			$this->db->select('test_price');      
			$this->db->from('test'); 
		...
		if($user_id){
			...
			if($item_id){
				if($item_id != 0){
					...
					$item_old_information = $this->getItemInformation($item_id);
